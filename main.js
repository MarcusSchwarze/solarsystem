/* --------------------------------------------------
   ...
   Hauptskript: Szene aufbauen, Animation steuern
-------------------------------------------------- */
...
import { AU, DEG, INC_SCALE, sphere, kepler, tex } from './utils.js?v=14';
import { planets }                                 from './planets.js?v=14';
...
(function(){
  ...
  for(let i=0; i<5000; i++){
    mx.makeTranslation((Math.random()-0.5)*3500,
...
const SIZE_SCALE = 3;   // Planetenradius‑Faktor
const bodies = [];
planets.forEach(p => {
  ...
  /* Ringe (z. B. Saturn) */
  if(p.rings){
    const rGeo = new THREE.RingGeometry(p.rings.inner * SIZE_SCALE, p.rings.outer * SIZE_SCALE, 128);
...
/* ── Flug‑/Follow‑Variablen ───────────────────────── */
let fly = false, align = false, follow = true;
