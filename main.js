/* Hauptlogik: Aufbau, Animation, HUD */
...
import { AU, DEG, INC_SCALE, sphere, kepler } from './utils.js';
...
(function(){
  ...
  for(let i=0;i<5000;i++){
    mx.makeTranslation((Math.random()-.5)*3500,
                       (Math.random()-.5)*3500,
                       (Math.random()-.5)*3500);
...
scene.add(sun);
sun.add(new CSS2DObject(Object.assign(document.createElement('div'),
          {className:'label', textContent:'Sonne'})));

/* Körper erzeugen ------------------------------*/
const SIZE_SCALE = 3;           // Planeten künstlich größer
const bodies = [];
planets.forEach(p=>{
  ...
  pMesh.add(new CSS2DObject(Object.assign(document.createElement('div'),
  ...
  if(p.rings){
    ...
    const rMat = new THREE.MeshBasicMaterial({
      map:      sphere(1).material.map, // Dummy, wird gleich ersetzt
      side:     THREE.DoubleSide,
      transparent:true
    });
    rMat.map      = (await import('./utils.js')).tex(p.rings.color);
    rMat.alphaMap = (await import('./utils.js')).tex(p.rings.alpha);
  ...
  p.m.forEach(m=>{
    ...
    mMesh.add(new CSS2DObject(Object.assign(document.createElement('div'),
...
bodies.forEach(b=>{
  ...
  b.moons.forEach(m=>{
    sel.add(new Option('  '+m.n, b.p.n+'|'+m.n));
  });
});
sel.selectedIndex = 2;
...
/* Flug-, Align- und Follow-Variablen */
let fly   = false,
    align = false,
    follow= true;
let tFlyStart, camStart, followRadius=50, followAngle=0,
    followTarget, desiredOffset;
...
document.getElementById('flyBtn').onclick = ()=>{
  ...
  fly = true; align = false; follow = true; followAngle = 0; desiredOffset = null;
...
window.addEventListener('keydown', e=>{
  if(e.key === 'Escape'){ follow = false; align = false; }
});

/* Animation ------------------------------------*/
const J2000 = Date.UTC(2000,0,1,12);         // 1 Jan 2000 12 UTC
const MSDAY = 86_400_000;                    // ms pro Tag
